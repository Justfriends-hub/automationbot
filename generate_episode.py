...
def render_manim(config_path: str, timing_path: str, workdir: str) -> str:
    ...
    media_dir = os.path.join("survive_history", workdir, "media", "videos", "survive_history_scene")
    for root, _dirs, files in os.walk(media_dir):
        for f in files:
            if f == "Episode.mp4":
                return os.path.join(root, f)
    raise FileNotFoundError(f"Manim did not produce Episode.mp4 under {media_dir}")
